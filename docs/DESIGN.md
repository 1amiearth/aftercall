...
- **คัดลอกไปวางใน Slack** (`slackMessage()` ใน `summary.js`): โพสต์สั้นสำหรับทีม ไม่ใช่ `summary.md` ทั้งฉบับ มีแค่สรุปสั้น, ตัดสินใจแล้ว, ใครทำอะไร (ชื่อคนรับตัวหนานำหน้า เดดไลน์ที่ไม่ระบุตัดทิ้ง คนรับที่ไม่ระบุเป็น "ยังไม่มีคนรับ") และต้องตามต่อ (ความเสี่ยงรวมกับคำถาม) ตัดหัวข้อที่คุยและนาทีในวิดีโอทิ้ง หัวข้อว่างไม่แสดง คัดลอกเป็น HTML คู่ข้อความ Slack markup (`*ตัวหนา*`, `•`) เพราะ Slack แสดงหัวข้อ `##` และตาราง Markdown ไม่ได้ ใช้กับสรุปถึงตอนนี้ด้วย ผลสรุป JSON เก็บใน `latest.summary.result`
- **คำสั่งเพิ่มเติมสำหรับสรุป** (`aiNotes` ในตั้งค่า ไม่เกิน 1,000 ตัวอักษร): ต่อท้าย system prompt ใต้กฎหลัก (`systemPrompt()` ใน `summary.js`) บอกได้ว่าเน้นอะไร หรือขอหัวข้อเสริมที่อยากได้ทุกครั้ง แต่ไม่เปลี่ยน 6 หัวข้อหลัก เพราะ schema บังคับอยู่
...
ไม่พึ่งพรอมต์อย่างเดียว ส่ง schema ให้ CLI บังคับ (`--json-schema` ของ Claude Code, `--output-schema` ของ Codex) ให้โมเดลตอบเป็น JSON ตามโครง 6 หัวข้อหลัก บวกหัวข้อเสริม แล้วส่วนขยายแปลงเป็น `summary.md` เอง หัวข้อหลักจึงครบและเรียงเหมือนกันทุกครั้ง

**หัวข้อเสริม** (`extra_sections`, เพิ่ม 2026-09-25): เรื่องที่ทีมต้องรู้แต่ไม่เข้า 6 หัวข้อ เช่น นัดครั้งถัดไป (วัน เวลา ใครเข้า เตรียมอะไร), ลำดับขั้นต่อไป, ตัวเลขหรือวันสำคัญของโปรเจกต์, เอกสารหรือลิงก์ที่อ้างถึง โมเดลตั้งชื่อหัวข้อเองตามภาษาประชุม สร้างเฉพาะเมื่อมีการพูดถึงจริง ห้ามซ้ำกับหัวข้อหลัก แสดงต่อจาก action items ทั้งใน `summary.md`, `review.html` และโพสต์ Slack ส่วนสรุปที่เก็บไว้ก่อนมีหัวข้อเสริมจะได้เป็นรายการว่าง (`extraSections()`)

พรอมต์เขียนให้คนที่ไม่ได้เข้าประชุมอ่านเข้าใจโดยไม่ต้องเปิด transcript: ทุกข้อต้องบอกว่าเป็นเรื่องของ feature / ระบบ / ลูกค้า / เอกสารไหน และเหตุผลถ้ามีคนพูดถึง ห้ามเขียนลอยๆ เช่น "คุยเรื่องปัญหา"
...
+  หัวข้อเสริมตามที่คุยจริง เช่น นัดครั้งถัดไป (ไม่บังคับ)
```
