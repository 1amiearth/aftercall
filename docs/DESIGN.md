...
`tabCapture.getMediaStreamId` ต้องให้ผู้ใช้เรียกใช้ส่วนขยายบนแท็บนั้นก่อน ยังไม่ยืนยันว่าการกดปุ่มใน Side Panel นับเป็นการเรียกใช้แบบเดียวกับกดไอคอนส่วนขยายหรือไม่ ต้องทำ prototype ข้อนี้ก่อน ถ้าไม่นับ ให้เริ่มอัดจากการกดไอคอนส่วนขยาย
...
Side Panel มีปุ่ม **สรุปใหม่** ที่เอา transcript ของประชุมล่าสุดจาก `chrome.storage.session` ไปสรุปอีกครั้ง เปลี่ยนโมเดลก่อนกดได้
...
Caption มีเมื่อผู้ใช้เปิด Live captions (CC) ใน Meet ถ้ายังไม่เปิด ให้ Side Panel บอกให้เปิดก่อน แล้วค่อยเริ่มเก็บข้อความ
...
- ข้อความแจ้งในแชทควรบอกว่าอัดอะไร และจะส่ง transcript ไปสรุปด้วย AI
...
- WebM จาก `MediaRecorder` ไม่มีความยาวคลิปในไฟล์ บางโปรแกรมเล่นจึงลากแถบเวลาไม่ได้ ถ้าจะแก้ ให้ใช้ `ts-ebml` เขียนข้อมูลส่วนหัวใหม่หลังอัดเสร็จ
- วิดีโอเก็บเป็น chunk ในหน่วยความจำของ Offscreen จนกว่าจะกด Stop ถ้าประชุมยาวมาก หน่วยความจำจะโตตามความยาวคลิป
